...
RSpec.describe I18n::Lint do
  ...
  it "allows registering rules" do
    test_rule_class = Class.new(I18n::Lint::Rule) do
      def on_segment(*); end
    end
    expect(I18n::Lint.register_rule(test_rule_class)).to be_a(I18n::Lint::RuleType)
  end
  ...
  context "with a class rule with a description" do
    ...
    it "finds offences for the rule" do
      ...
      expect(I18n::Lint.lint(examples.join("class/*.yml"), source_locale: "fr")).to contain_exactly_offences(
        I18n::Lint::Offence.new("Test/Description", examples.join("class/bad.yml").to_s, nil, nil, nil, nil,
  ...
  context "with a class rule that adds a custom message" do
    ...
    it "finds offences for the rule" do
      examples = Pathname.new(File.expand_path("../examples/", __dir__))
      expect(I18n::Lint.lint(examples.join("class/*.yml"), source_locale: "fr")).to contain_exactly_offences(
        I18n::Lint::Offence.new("Test/Message", examples.join("class/bad.yml").to_s, nil, nil, nil, nil,
